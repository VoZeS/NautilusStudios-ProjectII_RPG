...
bool NPC::Draw()
{
	...
	case 3: app->render->DrawTexture(app->entities->herrero, METERS_TO_PIXELS(position.x - 30.0f), METERS_TO_PIXELS(position.y - 42.0f), &rect);
		break;
	case 4: app->render->DrawTexture(app->entities->sabio, METERS_TO_PIXELS(position.x - 30.0f), METERS_TO_PIXELS(position.y - 42.0f), &rect);
		break;
	case 5: app->render->DrawTexture(app->entities->aldeano, METERS_TO_PIXELS(position.x - 30.0f), METERS_TO_PIXELS(position.y - 42.0f), &rect);
		break;
	case 7: app->render->DrawTexture(app->entities->granjero, METERS_TO_PIXELS(position.x - 30.0f), METERS_TO_PIXELS(position.y - 42.0f), &rect);
		break;
	default:
		break;
	}
